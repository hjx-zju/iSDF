...
class Custom_Entropy_Linear(nn.Module):
    def __init__(self, in_features, out_features, l=1, weight_decoder=None):
        super().__init__()
        self.l = l
        ...
        self.weight = nn.Parameter(
            th.randn(in_features * out_features, l),
            requires_grad=True,
        )
        nn.init.xavier_uniform_(self.weight.view(out_features, in_features))
        self.bias = nn.Parameter(th.zeros(out_features, 1), requires_grad=True)

        self.kernel_log_step = nn.Parameter(th.tensor(-4.0), requires_grad=True)
        # self.kernel_offset = nn.Parameter(th.tensor(0.0), requires_grad=True)

        self.entropy_bottleneck_w = EntropyBottleneck(l)
        # if bias_decoder:
        # self.entropy_bottleneck_b = EntropyBottleneck(l)
        # self.bias_log_step = nn.Parameter(-4.0, requires_grad=True)
        # self.bias_offset = nn.Parameter(0.0, requires_grad=True)
    ...
    def update(self, force=False, update_quantiles=False):
        self.entropy_bottleneck_w.update(
    ...
    def get_compressed_params_size(self):
        ...
        self.to("cpu")
        strings = self.entropy_bottleneck_w.compress(
            self.weight.T.unsqueeze(0) / th.exp(self.kernel_log_step)
        )
        ...
        parameters_size += self.bias.numel() * 4
        # self.entropy_bottleneck_w.train()
        self.to(device)
        return parameters_size, tables_size
    
    def get_decompressed_parameters(self):
        w=self._transform_dense_weight(self.weight,self.kernel_log_step,quantized=False)
        return [w.clone.detach(),self.bias.clone.detach()]
    
    def get_model_size(self):
        return self.weight.numel() * 4 + self.bias.numel() * 4
    
    def _transform_dense_weight(self, weight, lop_step, quantized=True):
        step = th.exp(lop_step)
        if not quantized:
            weight = quantize_ste(weight / step)
        return weight * step
    ...
    def compress(self):
        ...
        b += self.kernel_log_step.detach().cpu().numpy().tobytes()
        b += int.to_bytes(len(strings_w[0]), 4, byteorder="little")
        b += strings_w[0]

        b += self.bias.detach().cpu().numpy().tobytes()
    ...
    def decompress(self, b):
        ...
        # kernel_log_step, b = get_next_tensor(b, dtype=np.float32)
        kernel_log_step=th.from_numpy(np.frombuffer(b[:4], dtype=np.float32).copy())
        b=b[4:]
        ...
        self.kernel_log_step.data = kernel_log_step
        w = self.entropy_bottleneck_w.decompress(strings_w, self.weight.shape)[0][0]
        self.weight.data = self._transform_dense_weight(
            w, self.kernel_log_step, quantized=True
        )

        # decode bias
        b_len = self.bias.numel() * 4
        self.bias.data = th.from_numpy(
            np.frombuffer(b[:b_len], dtype=np.float32).copy()
        ).view(self.bias.shape)
        b = b[b_len:]
    ...
    def forward(self, x):
        if self.training:
            _, likelyhoods_w = self.entropy_bottleneck_w(
                self.weight.T.unsqueeze(0) / th.exp(self.kernel_log_step), training=True
            )
        else:
            _, likelyhoods_w = self.entropy_bottleneck_w(
                self.weight.T.unsqueeze(0) / th.exp(self.kernel_log_step), training=False
            )
        
        w = self._transform_dense_weight(
            self.weight, self.kernel_log_step, quantized=False
        )
        b = self.bias

        w = w.view(self.out_features, self.in_features)
        b = b.view(self.out_features)
        rate_w = -th.log2(likelyhoods_w).sum() / w.numel()
        return F.linear(x, w, b), rate_w
...
class EntropyLayer(nn.Module):
    ...
    def __init__(
        ...
        self.weight = nn.Parameter(
            th.randn(in_features * out_features, l), requires_grad=True
        )
        ...
        if self.bias_decoder:
            self.entropy_bottleneck_b = CustomEntropyBottleneck(l)
    ...
    def update(self, force=False, super_up=True):
        ...
        # print("update quantiles",self.entropy_bottleneck_w.quantiles)
        if self.bias_decoder:
    ...
    def get_compressed_params_size(self):
        ...
        if self.bias_decoder:
            strings = self.entropy_bottleneck_b.compress(
            ...
            # no compression for bias, return origin bias size
            parameters_size += self.bias.numel() * 4
        self.entropy_bottleneck_w.train()
        self.to(device)
    ...
    def compress(self):
        ...
        if self.bias_decoder:
            ...
            b += self.ema_b.detach().cpu().numpy().tobytes()
    ...
    def decompress(self, b):
        ...
        self.ema_w.data = self.entropy_bottleneck_w.decompress(
            strings_w, self.ema_w.shape
        )[0][0]
        self.weight.data = self.ema_w.data
        ...
        if self.bias_decoder:
            ...
            self.ema_b.data = self.entropy_bottleneck_b.decompress(
                strings_b, self.ema_b.shape
            )[0][0]
        else:
            ema_b_len = self.ema_b.numel() * 4
            self.ema_b.data = th.from_numpy(
                np.frombuffer(b[:ema_b_len], dtype=np.float32).copy()
            ).view(self.ema_b.shape)
            self.bias.data = self.ema_b.data
            b = b[ema_b_len:]
        self.to(device)
        return b

    def get_decompressed_parameters(self):
        w_hat = self.weight_decoder(self.ema_w)
        if self.bias_decoder:
            b_hat = self.bias_decoder(self.ema_b)
        else:
            b_hat = self.ema_b
        return w_hat, b_hat
    ...
    def get_ema_and_rate(self):
        if self.training:
            ...
            self.ema_w = nn.Parameter(
                self.ema_decay * detatched_w + (1 - self.ema_decay) * self.weight
            )
            if self.bias_decoder:
                detatched_b = self.ema_b.detach()
                self.ema_b = nn.Parameter(
                    self.ema_decay * detatched_b + (1 - self.ema_decay) * self.bias
                )
            else:
                self.ema_b = self.bias
        ...
        if self.training:
            ...
            b = self.ema_b
            w = self.ema_w
        # w_hat, likelyhoods_w = self.entropy_bottleneck_w(w.T.unsqueeze(0))
        # w_hat=w_hat.T
...
class EntropyConv2d(EntropyLayer):
    ...
    def __init__(
        ...
        self.extra_args = {"padding": padding, "stride": stride}
        self.layer_func = F.conv2d

    def init_weights(self):
        nn.init.xavier_uniform_(
            self.weight.view(self.out_features, self.in_features, *self.kernel_size)
        )
        self.weight.data /= th.exp(th.tensor(-4))
